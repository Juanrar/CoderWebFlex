...
*Notas de la sesión 2026-06-28 (estilos avanzados: transiciones, transformaciones, `@keyframes`):* se cerró entero el bloque de estilos avanzados. Conceptos trabajados: **`display` NO se anima** (es una propiedad discreta: salta de `none` a `block` sin pasos intermedios) → la visibilidad de un elemento que se quiere "fade" se maneja con `opacity` (continua, sí animable). El **`transition` va en la regla base** del elemento (su estado de reposo), NO en `:hover`/`:checked` — ahí vive la "instrucción permanente" y por eso anima en ambas direcciones (entrar y salir); si se pone solo en el estado activo, abre suave pero cierra de golpe. Diferencia clave: `opacity: 0` es **invisible pero presente** (sigue ocupando espacio y atrapando clicks), a diferencia de `display:none` que lo saca de todo → para que el menú cerrado no capture clicks fantasma se suma `visibility: hidden`↔`visible`, **incluyéndola en el `transition`** (truco: si no, `visibility` salta de golpe y se come el fade de salida). `transform`/`opacity` son las propiedades más baratas de animar (GPU, sin recalcular layout). `@keyframes`: se define el "guion" (`from`/`to` o `0%`/`100%`) y recién se activa al aplicarlo con la propiedad `animation` (definirlo solo = receta en el cajón); las animaciones CSS corren solas al renderizar (no necesitan disparador ni JS). Truco elegante para las barras de skills: **omitir el `to`** y poner solo `from { width: 0 }` → el navegador usa como estado final el `width` que ya tiene cada `.bar--XX`, así un único `@keyframes` sirve para las 6 barras con anchos distintos. Bugs recurrentes de esta sesión: **`transition: 0.3`** sin unidad de tiempo (un tiempo CSS necesita `s`/`ms`; sin unidad = valor inválido = propiedad muerta — mismo patrón que `aria-lebel`/`name="project"`); aplicar el efecto al **selector equivocado** (`&__project` no existe en `_header.scss`, era el `ul` del dropdown); e intentar `to { width: var(--dist) }` con una **variable nunca definida** (más complejo y roto, cuando el truco de omitir el `to` ya lo resolvía). El hover lindo terminó en las tarjetas del portfolio (`&__project:hover` con `scale` + `outline` + `opacity`), no en los links del dropdown.

#### ✅ Bootstrap y librerías (COMPLETADO 2026-06-28)
...
- [x] **Override de identidad COMPLETO (2026-06-28)**: el `.btn-primary` ya no tiene nada del azul de fábrica. Se hizo redefiniendo las **CSS custom properties** de Bootstrap 5.3 (`--bs-btn-bg`, `--bs-btn-border-color`, `--bs-btn-hover-bg`, `--bs-btn-hover-border-color`, `--bs-btn-active-bg`, `--bs-btn-active-border-color`) con la paleta propia, en un partial dedicado `scss/base/_bootstrap.scss` (linkeado en `main.scss`, sacado de adentro de `.form`). El hover usa `color.scale($color-primario, $lightness: -15%)` para oscurecer. Conceptos: CSS custom properties (`--bs-*`, runtime/cascada) ≠ variables SASS (`$`, compile-time) → se cruzan con interpolación `#{...}`; los botones 5.3 leen `var(--bs-btn-*)`, así que pisar la variable se propaga a todos los estados (en vez de pelear especificidad estado por estado); para empate de especificidad, el `<link>` de `styles.css` debe ir DESPUÉS del de Bootstrap. Detalle estético menor pendiente: el `hover-border-color` quedó en el primario brillante (no oscurecido como el fondo). Bugs de la sesión: partial sin `@use` en `main.scss` (receta en el cajón, mismo patrón que `@keyframes`), propiedad declarada dos veces (gana la última, la otra queda muerta), y pisar sin querer `--bs-btn-hover-border-color` al duplicar el nombre.
- [x] **Librería de animación AOS linkeada y aplicada (2026-06-28)**: CSS en el `<head>` y JS + `AOS.init({duration, once:true, offset})` antes de `</body>` en las **6 páginas** (URL CDN absoluta → sin problema de rutas relativas). Aplicada en el `index` (tarjetas del portfolio con `fade-up` y stagger vía `data-aos-delay` 0/100/200/300, títulos de sección y bloque de perfil). Conceptos: AOS anima al entrar en viewport, se activa con el atributo `data-aos` por elemento (sin atributo = librería cargada pero "dormida"); criterio de diseño = animar para puntuar/guiar, no decorar (no tocar nav ni las barras de skills que ya tienen su `@keyframes`); cuidado con elementos above-the-fold (arrancan en `opacity:0` → posible flash). Pendiente opcional: las 5 subpáginas tienen AOS linkeado pero SIN ningún `data-aos` (dormido) → sumar uno al `<figure>`/`<h1>` de cada una para que también animen.
...
- [x] **Optimizar imágenes pesadas (2026-06-28)**: `demoFeeder` 922 KB → **85 KB WebP** (−91%) y `demoDance` 686 KB → **42 KB WebP** (−94%), exportadas con Squoosh. Los `.png` viejos se reemplazaron (sin huérfanos) y los `src` de `feeder.html`/`dance.html` ya apuntan a `.webp`. Concepto: el peso lo dominaba el **formato**, no las dimensiones (`demoFeeder` era la de MENOS píxeles pero la más pesada, por ser PNG lossless guardando una captura compleja). Foto de perfil `IMG-20211212-WA0074.jpg` ~236 KB ya estaba ok; `demoAssembler.webm` ~175 KB ok.
- [ ] **(Opcional) `demoBeatmatch.png`** sigue en PNG (~169 KB, 1342×1424px). Peso ya sano, pero pasarla a WebP la dejaría en ~50-70 KB y los 3 demos consistentes.
